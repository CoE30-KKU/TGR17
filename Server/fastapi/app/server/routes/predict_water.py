...
from server.database.predict_water_data import (
    add_water,
    delete_water,
    retrieve_water,
    retrieve_waters,
...
from server.models.predict_water import (
    ErrorResponseModel,
    ResponseModel,
    PredictWaterSchema,
    UpdatePredictWaterModel,
)

router = APIRouter()

@router.post("/", response_description="Add Predict Water data into the database")
async def add_water_data(water: PredictWaterSchema = Body(...)):
    water = jsonable_encoder(water)
    new_water = await add_water(water)
    return ResponseModel(new_water, "Water added successfully.")

@router.get("/", response_description="Retrieve Predict Water data")
...
@router.get("/{id}", response_description="Retrieve Predict Water data")
...
@router.put("/{id}")
async def update_water_data(id: str, req: UpdatePredictWaterModel = Body(...)):
...
@router.delete("/{id}", response_description="Delete Predict Water")
